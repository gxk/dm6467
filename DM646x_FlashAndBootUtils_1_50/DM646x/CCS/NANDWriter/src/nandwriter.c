...
/* --------------------------------------------------------------------------
  ...
  DESC	      : CCS-based utility to flash the DM646x in preparation for 
                NAND booting
 ----------------------------------------------------------------------------- */
...
static Uint32 nandwriter()
{
  ...
  DEBUG_printString("Starting DM646x_NANDWriter.\r\n");
  ...
  if (strcmp(fileName,"none") != 0)
  {
    ...
    DEBUG_printString("Enter the UBL entry point (in hex): \n");
    DEBUG_readString(answer);
    gNandBoot.entryPoint = strtoul(answer, NULL, 16);
    fflush(stdin);

    if ( (gNandBoot.entryPoint < 0x0020) || (gNandBoot.entryPoint >= (0x0020 + UBL_IMAGE_SIZE)) )
    {
      DEBUG_printString("\tWARNING: Entry point not in acceptable range - using default 0x0100.\r\n");
